...
function isLightColor(color){
  ...
  return red*.2126+green*.7152+blue*.0722>180;
}
...
export default function BorderGlow({children,className='',edgeSensitivity=30,glowColor='0 0 88',backgroundColor='#000000',borderRadius=28,glowRadius=40,glowIntensity=1,coneSpread=25,animated=false,colors=['#f5f5f5','#a3a3a3','#525252'],fillOpacity=.5}){
  ...
  const handlePointerMove=useCallback(e=>{const card=cardRef.current;if(!card)return;const rect=card.getBoundingClientRect(),x=e.clientX-rect.left,y=e.clientY-rect.top,edge=getEdgeProximity(card,x,y),angle=getCursorAngle(card,x,y);const header=card.classList.contains('engineering-header-glow');card.style.setProperty('--edge-proximity',`${((header?Math.max(.8,edge):edge)*100).toFixed(3)}`);if(header){card.style.setProperty('--header-pointer-x',`${x}px`);card.style.setProperty('--header-pointer-y',`${y}px`);}card.style.setProperty('--cursor-angle',`${angle.toFixed(3)}deg`)},[getEdgeProximity,getCursorAngle]);
